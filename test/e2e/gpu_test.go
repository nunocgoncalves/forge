...
func dumpGPUDiagnostics(t *testing.T, ip, keyPath, kcPath string) {
	...
	k8sCmds := []k8sCmd{
		...
		{[]string{"logs", "-n", "gpu-operator", "-l", "app=nvidia-cuda-validator", "-c", "cuda-validation", "--tail=100"}, "sudo k3s kubectl logs -n gpu-operator -l app=nvidia-cuda-validator -c cuda-validation --tail=100"},
		{[]string{"logs", "-n", "gpu-operator", "-l", "app=nvidia-cuda-validator", "-c", "cuda-validation", "--previous", "--tail=100"}, "sudo k3s kubectl logs -n gpu-operator -l app=nvidia-cuda-validator -c cuda-validation --previous --tail=100"},
