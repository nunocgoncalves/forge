...
func dumpGPUDiagnostics(t *testing.T, ip, keyPath, kcPath string) {
	...
	k8sCmds := []k8sCmd{
		...
		{[]string{"describe", "pod", "-n", "gpu-operator", "-l", "app=nvidia-cuda-validator"}, "sudo k3s kubectl describe pod -n gpu-operator -l app=nvidia-cuda-validator"},
		{[]string{"logs", "-n", "gpu-operator", "-l", "app=nvidia-cuda-validator", "--all-containers=true", "--tail=100"}, "sudo k3s kubectl logs -n gpu-operator -l app=nvidia-cuda-validator --all-containers=true --tail=100"},
		{[]string{"logs", "-n", "gpu-operator", "-l", "app=nvidia-cuda-validator", "--all-containers=true", "--previous", "--tail=100"}, "sudo k3s kubectl logs -n gpu-operator -l app=nvidia-cuda-validator --all-containers=true --previous --tail=100"},
